...
Using B-tree functions available in https://www.dropbox.com/sh/rq3qtyizpgjnr2c/AAAA8ecXHI_A8r4wK9cJlwUOa/ArvoreB?dl=0 by professor Renato Mauro.
*/
...
int main(int argc, char** argv)
{
    ...
    FILE *arquivoNis = fopen("bolsa.csv", "r");
    fgets(linha,2048,arquivoNis);
    posicaoOriginal = ftell(arquivoNis);
    fgets(linha,2048,arquivoNis);

    while(!feof(arquivoNis)) {
        ...
        while (campo)
        {
            if (coluna == 7)
            {
                ...
                sprintf(chave,"%08d",i);
                posicaoArvore=i;
                ArvoreB_Insere(arvore, chave, posicaoOriginal);      //inserir na arvore o nis
                printf("NIS => %s esta na arvore B na posicao: %ld\n", nis, posicaoArvore);       //Mostar o Nis e a sua posicao na arvore B.
                i++;
            }
            coluna++;
        ...
        n++;
            ...
            //condicional para parar na leitura de 10 chaves do arquivo csv para arvoreB
            if(n == 10)
            {
            break;
            }
    }
    ArvoreB_PrintDebug(arvore);  // roda funcao debug da arvoreB 
